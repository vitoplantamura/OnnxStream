//
// The Stable Diffusion implementation in this file is based on these two projects:
...
static inline ncnn::Mat CFGDenoiser_CompVisDenoiser(ncnn::Net& net, float const* log_sigmas, ncnn::Mat& input, float sigma, ncnn::Mat& cond, ncnn::Mat& uncond)
{
    ...
    auto run_inference = [&net, &input, &t_mat, &c_in_mat, &c_out_mat](ncnn::Mat& output, ncnn::Mat& cond_mat) {
...
#if USE_ONNXSTREAM
        {
            ...
            tensor_vector<float> cc_v((float*)cond_mat, (float*)cond_mat + cond_mat.total());
...
inline static std::pair<ncnn::Mat, ncnn::Mat> prompt_solver(std::string const& prompt_positive, std::string const& prompt_negative)
{
    ...
    ncnn::Net net;
    {
        ...
        // 读取tokenizer字典
        std::ifstream infile;
        std::string pathname = g_main_args.m_path_with_slash + "tokenizer/vocab.txt";
...
int main(int argc, char** argv)
{
    ...
    for (int i = 1; i < argc; i++)
    {
        ...
        else
        {
            ...
            printf("--decoder-calibrate Calibrates the quantized version of the VAE decoder.\n");
            printf("--decoder-fp16      During inference, uses the FP16 version of the VAE decoder.\n");
            printf("--rpi               Configures the models to run on a Raspberry Pi Zero 2.\n");
